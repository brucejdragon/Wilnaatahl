...
type private TestRelation<'TTrait when 'TTrait :> ITrait>(createTrait: unit -> 'TTrait) =
    ...
    interface IRelation<'TTrait> with
        ...
        member this.On(entity: EntityId) =
...
module private World =
    ...
    let targetsFor (relation: IRelation<'TTrait>) entity world =
        ...
        let subjectHasTrait (kvp: KeyValuePair<int, 'TTrait>) =
            if world |> hasTrait kvp.Value entity then
                Some(EntityId kvp.Key)
            else
                None

        relationImpl.TargetToTraitMap |> Seq.choose subjectHasTrait
    ...
    let query where world =
        ...
        let collect acc traitSpec =
            match traitSpec with
            | With someTrait ->
                { acc with
...
type TestWorld() =
    ...
    interface IEntityOperations with
        member _.Add someTrait entity = world |> addTrait someTrait entity
        member _.Destroy entity = world |> destroy entity

        member _.Get valueTrait entity =
            world |> getTraitValue valueTrait entity

        member _.Has someTrait entity = world |> hasTrait someTrait entity
        member _.Remove someTrait entity = world |> removeTrait someTrait entity

        member _.Set valueTrait value entity =
            world |> setTraitValue valueTrait value entity

        member _.SetWith valueTrait update entity =
            world |> setTraitValueWith valueTrait update entity

        member _.TargetFor relation entity = world |> targetFor relation entity
        member _.TargetsFor relation entity = world |> targetsFor relation entity
