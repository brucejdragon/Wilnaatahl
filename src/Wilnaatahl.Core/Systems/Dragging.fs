...
let private handleDragStart (world: IWorld) =
    if not (world.Has DragStartEvent) then
        ...
        | Some(nodeEntity, origin) ->
            world.Spawn((Dragging.On nodeEntity).Val origin) |> ignore
            world
        | None ->
...
let private handleDrag (world: IWorld) =
    ...
    | Some move ->
        ...
        | Some(dragEntity, nodeEntity, origin) ->
            ...
            | Some oldPosition ->
                let nx = origin.x + move.x
                let ny = origin.y + move.y
                let nz = origin.z + move.z

                let dx = nx - oldPosition.x
                let dy = ny - oldPosition.y
                let dz = nz - oldPosition.z

                world.QueryTrait(Position, With Selected).UpdateEach
                <| fun (pos, _) ->
                    pos.x <- pos.x + dx
                    pos.y <- pos.y + dy
                    pos.z <- pos.z + dz
