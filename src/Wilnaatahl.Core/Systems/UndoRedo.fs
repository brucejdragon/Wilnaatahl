...
module private Snapshot =
    ...
    let getSnapshot world entity =
        { World = world; Entity = entity; HasItems = false }

    let capture entity position snapshot =
        entity |> addWith (SnapshottedBy.On snapshot.Entity) position
        snapshot.HasItems <- true

    let destroy snapshot = snapshot.Entity |> destroy

    let getEntities snapshot =
        snapshot.World.Query(With(SnapshottedBy.On snapshot.Entity))

    let getSavedPositionFor entity snapshot =
        entity |> get (SnapshottedBy.On snapshot.Entity)
...
let private handleDragStart (world: IWorld) (undoStack: Stack<EntityId>) =
    ...
    if not (world.Has DragStartEvent) then
        ...
        world.QueryTrait(Position, With Selected, Not [| With TargetPosition |]).ForEach
...
let private handleDragEnd (world: IWorld) (redoStack: Stack<EntityId>) =
    if not (world.Has DragEndEvent) then
        ...
        let draggingButNotAnimating =
            world.Query(With Selected, Not [| With TargetPosition |])
...
let private handleButtonClicked (world: IWorld) (toStack: Stack<EntityId>) (fromStack: Stack<EntityId>) =
    let snapshot = getSnapshot world (fromStack.Pop()) // ASSUMPTION: This function won't be called when fromStack is empty.
