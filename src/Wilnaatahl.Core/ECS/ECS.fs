...
module Extensions =
    ...
    type IQueryResult<'T, 'TMutable> with
        /// Applies the given mapping function over the results of the query. Note that this fully
    ...
    type IWorld with
        ...
        member this.QueryFirstTarget<'T, 'TTrait when 'TTrait :> IValueTrait<'T>>
            ...
            | Some entity ->
                ...
                | Some target ->
                    match entity |> get (relation.On target) with
                    | Some value -> Some(entity, target, value)
                    | None ->
                ...
                | None ->
                    ...
                    // in between QueryFirst and targetFor, which would be a concurrency bug in the app.
